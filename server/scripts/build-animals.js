...
async function main() {
  ...
  const animals = items.filter(i => i.inventoryCategory === 'AnimalProduct');
  const rows = animals.map(it => {
    return {
  ...
  const out = path.join(__dirname, '..', 'data', 'animal-products.json');
  fs.mkdirSync(path.dirname(out), { recursive: true });
  fs.writeFileSync(out, JSON.stringify(rows, null, 2) + '\n');
  console.log(`Wrote ${rows.length} animal products to ${out}`);
}
