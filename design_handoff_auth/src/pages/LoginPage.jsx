...
// Updated: added Google sign-in button + link to /register.
// Replaces client/src/pages/LoginPage.jsx in full.
          ...
          {results.map((r, i) => {
            ...
            return (
              ...
              onMouseLeave={e => (e.currentTarget.style.background = 'white')}>
                ...
                <span style={{ fontSize: 10.5, fontWeight: 600, color: tc.color, background: tc.bg, padding: '2px 8px', borderRadius: 999, flexShrink: 0 }}>{tc.label}</span>
              </button>
            );
          })}
...
function LoginForm({ onEnter }) {
  ...
  function handleSignIn(ev) {
    ...
    if (!validate()) return;
    setLoading(true);
    // TODO: replace with your real auth call, e.g. supabase.auth.signInWithPassword(...)
    ...
    <div className="ci-split" style={{ display: 'flex', height: '100vh', overflow: 'hidden' }}>
          ...
          <div style={{ animation: 'fadeUp 0.5s ease 0.2s forwards' }}>
            ...
            <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
              {['Best spring crops', 'Water Mine', "Lily's gifts", 'Cranberry', 'Ancient Relic'].map(q => (
                <button key={q} onClick={enter} style={{
                  padding: '5px 13px', borderRadius: 999, background: 'rgba(255,255,255,0.07)',
                  border: '1px solid rgba(255,255,255,0.13)', color: 'rgba(255,255,255,0.65)',
      ...
      <div className="ci-form" style={{
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 13.5, color: '#6b7a74', lineHeight: 1.55 }}>Sign in to save progress, favourites &amp; custom notes.</p>
        </div>
