...
function ForagingPage({ density }) {
  const [season, setSeason] = React.useState('');
  const [area,   setArea]   = React.useState('');
  const [search, setSearch] = React.useState('');
  const { foraging } = window.GameData;

  const seasons = ['spring','summer','fall','winter'];
  const areas   = React.useMemo(() => [...new Set(foraging.map(f => f.area))].sort(), [foraging]);
...
window.ForagingPage = ForagingPage;
