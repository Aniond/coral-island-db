...
function MineBanner({ mine }) {
  if (!mine) return null;
  const cfg     = window.mineConfig[mine];
  const unlock  = window.GameData.mineUnlocks[mine];
  if (!cfg) return null;
  return (
    <div style={{
...
function CavesPage({ density }) {
  const [mine,     setMine]     = React.useState('');
  const [itemType, setItemType] = React.useState('');
  const [search,   setSearch]   = React.useState('');
  const { caves } = window.GameData;
